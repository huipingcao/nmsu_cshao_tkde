...
def project_cnn_feature_combined_rf_lda_analysis(feature_matrix, y_vector, logger=None):
    ...
    map_attr_imp_index_matrix = []  #used to save the output objects
    predict = True
    skip_count = 0
    rf_time = 0
    lda_time = 0
...
def project_cnn_feature_rf_analysis(feature_matrix, y_vector, logger):
    num_instance, num_attribute, num_map = feature_matrix.shape
    map_attr_imp_matrix = [] # used to store all attribute importance from each map
    ...
    for i in range(0, num_map):
        ...
        feature_vector_norm, feature_value_vector, rf_model, averaged_acc = rf_feature_extraction(map_feature_matrix, y_vector)    
        overall_time = overall_time + time.time() - start_time
        if i == 0:
            logger.info(feature_vector_norm.shape)
